...
public class CommandOpOffers extends CommandBase {
    @Override
    @Nonnull
    ...
    public String getUsage(@Nullable ICommandSender sender) {
        return TranslationUtil.getRawTranslationString(sender, "commands.ge.opoffers.usage");
    }
    ...
    public void execute(@Nullable MinecraftServer server, @Nonnull ICommandSender sender, @Nullable String[] args) throws CommandException {
        List<Offer> buyOffers = Lists.newArrayList(ExchangeManager.getOffers(OfferType.BUY, null));
        List<Offer> sellOffers = Lists.newArrayList(ExchangeManager.getOffers(OfferType.SELL, null));
        ...
        if(buyOffers.isEmpty())
            sender.sendMessage(TranslationUtil.getTranslation(sender, "commands.ge.myoffers.no_buy_results").setStyle(TextStyles.RED));
        ...
        if(sellOffers.isEmpty())
            sender.sendMessage(TranslationUtil.getTranslation(sender, "commands.ge.myoffers.no_sell_results").setStyle(TextStyles.RED));

        if(buyOffers.isEmpty() && sellOffers.isEmpty())
            sender.sendMessage(TranslationUtil.getTranslation(sender, "commands.ge.common.not_buying_or_selling"));
        else
            ChatPageUtil.showPaginatedChat(sender, "/ge opoffers " + search + " %s", messages, page);
    }
